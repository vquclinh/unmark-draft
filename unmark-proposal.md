...
1. The **base stream** defines the token grid. All positions are indexed by `T(b(x))`.
   ...
   **For the PhoBERT branch this is literal.** `T(b(x))` means the frozen BPE tokenizer applied directly to the invariant stripped base stream, with **no post-strip word segmenter** between `b` and `T`. This is a deliberate departure from PhoBERT's standard usage, which expects pre-word-segmented Vietnamese: a scientifically provenance-verified probe compared four preprocessing pipelines and found that segmenting the *observed* text breaks base-grid invariance (9/18 cases), segmenting the *clean* text is not deployable because clean text does not exist at inference, and segmenting the stripped base recovers little of the clean segmentation for a fragmentation gain of 1.5424 versus 1.5674 with an identical unknown-token count. The trade-off is accepted openly: `RAW_BASE` is **not** claimed to match PhoBERT's pretraining preprocessing distribution, and that mismatch is a possible source of distribution shift. Baseline and clean-reference preprocessing is a separate question, locked when those pathways are implemented. Evidence: `docs/experiments/b3b0-input-contract-result.md`; decision: `docs/spec/decisions.md` D-B3B1A-001.
2. Each subword is assigned the channel labels of the syllable it belongs to, by tracking character offsets through tokenization.
   ...
   **For PhoBERT the offsets are reconstructed, not read.** The pinned slow tokenizer exposes no native offset mapping, so the character map is rebuilt from the tokenizer's own raw fastBPE pieces over the **same maximal non-whitespace units the tokenizer itself uses**, and then overlaid with the orthographic spans by character-range overlap. Reconstructing over linguistic spans instead was tried and refuted empirically: it reproduced the authoritative sequence for only 6 of 13 representative sentences, because punctuation, hyphens, URLs and e-mail addresses change the BPE segmentation of the whole chunk they sit in. Two consequences are load-bearing. First, the authoritative token grid always comes from `T(b(x))` directly; the character map is auxiliary and never redefines tokenization. Second, orthographic spans are metadata boundaries rather than tokenization boundaries, so a subword may draw from several regions, and such cases are recorded with their contributors rather than assigned a label by assumption. Evidence and policy: `docs/spec/decisions.md` D-B3B1B-001 and D-B3B1B-002.
